...
│   └── config.example.js   Template — copy to js/config.js and fill in real values (js/config.js is gitignored)
...
**First time only:** create your local config file (see
"Configuration" below) — `js/config.js` isn't in the repo, so the
dashboard won't be able to reach Supabase until it exists:

```bash
cp js/config.example.js js/config.js
# then edit js/config.js and fill in your real SUPABASE_URL / SUPABASE_KEY
```
...
server) — **including a real `js/config.js`**, since that file is
gitignored and won't come along automatically if you're deploying
straight from the repository. Nothing needs to be built or compiled
first.

## Configuration

The app needs two values to reach the Supabase database, read from
`js/config.js`:
...
**`js/config.js` is not committed to git** — it's listed in
`.gitignore` on purpose, as a source-control hygiene practice (see
"Security" below for why that's a hygiene choice rather than a claim
that the key itself is dangerous). What *is* committed is
`js/config.example.js`, a template with placeholder values. Create
your real config once with:

```bash
cp js/config.example.js js/config.js
```

then fill in the real `SUPABASE_URL`/`SUPABASE_KEY` in that copy.
Every other file in the app reads only `window.SUPABASE_URL` /
`window.SUPABASE_KEY` — nothing else needs to change if these values
are ever rotated or moved to a different mechanism (see
`.env.example` for how they'd map to a future build step, if one is
ever introduced).
...
- `SUPABASE_KEY` is Supabase's **publishable (anon) key** — the kind
  ...
  (RLS) policies, not by keeping this value secret. Keeping
  `js/config.js` out of git doesn't change that model — it's a
  hygiene practice (fewer places the value is written down, nothing
  to scrub from history later), not a fix for a vulnerability. The
  key is still visible to anyone who opens the deployed dashboard's
  browser network tab, exactly as intended by Supabase's design; no
  client-side app can prevent that.
- **The real security boundary is RLS, on every table** — not just
  the ones this project's SQL migration touches. `daily_sales` and
  `app_roles` are locked down (see `sql/001_lockdown_daily_sales_writes.sql`).
  `monthly_budgets`'s RLS status is unknown from this codebase alone —
  it's managed by a separate admin tool — and any other table in the
  same Supabase project is an unknown too. Worth an audit pass in the
  Supabase dashboard (Authentication → Policies) if that hasn't been
  done project-wide.
